...
class Pawn(Sprite):
    ...
    def __init__(   
            self, screen, pawn_image, explosion_images, 
            field, init_position, init_direction, speed):
        """ Create a new Pawn
        
            screen: 
                The screen on which the pawn lives (must be a 
                pygame Surface object, such as pygame.display)
            
            pawn_image: 
                Image (surface) object for the pawn
            ...
            field:
                A Rect specifying the 'playing field' boundaries.
                The Pawn will bounce off the 'walls' of this 
                field.
                
            init_position:
                A vec2d or a pair specifying the initial position
                of the pawn on the screen.
            
            init_direction:
                A vec2d or a pair specifying the initial direction
                of the pawn. Must have an angle that is a 
                multiple of 45 degres.
            
            speed: 
                Pawn speed, in pixels/millisecond (px/ms)
        """
        Sprite.__init__(self)
        ...
        # A vector specifying the Pawn's position on the screen
        #
        self.pos = vec2d(init_position)
        ...
        self.state = Pawn.ALIVE
        self.health = 15      
        
    def is_alive(self):
        return self.state in (Pawn.ALIVE, Pawn.EXPLODING) 
    
    def update(self, time_passed):
        """ Update the Pawn.
            time_passed: The time passed (in ms) since the previous update.
        """
        if self.state == Pawn.ALIVE:
            # Maybe it's time to change the direction ?
            #
            self._change_direction(time_passed)
            
            # Make the Pawn point in the correct direction.
            # Since our direction vector is in screen coordinates 
            # (i.e. right bottom is 1, 1), and rotate() rotates 
            # counter-clockwise, the angle must be inverted to 
            # work correctly.
        ...
        elif self.state == Pawn.EXPLODING:
            if self.explode_animation.active:
                self.explode_animation.update(time_passed)
            else:
                self.state = Pawn.DEAD
                self.kill()
        elif self.state == Pawn.DEAD:
            pass
    
    def draw(self):
        """ Blit the Pawn onto the screen that was provided in
            the constructor.
        """
        if self.state == Pawn.ALIVE:
            # The Pawn image is placed at self.pos. To allow for 
            # smooth movement even when the Pawn rotates and the 
            # image size changes, its placement is always centered.
            #
            self.draw_rect = self.image.get_rect().move(
                self.pos.x - self.image_w / 2, 
                self.pos.y - self.image_h / 2)
    ...
    # States the Pawn can be in.
    #
    # ALIVE: The Pawn is roaming around the screen
    # EXPLODING: 
    #   The Pawn is now exploding, just a moment before dying.
    # DEAD: The Pawn is dead and inactive
    #
    (ALIVE, EXPLODING, DEAD) = range(3)
    ...
    def _point_is_inside(self, point):
        """ Is the point (given as a vec2d) inside our Pawn's
            body?
        """
        img_point = point - vec2d(  
            int(self.pos.x - self.image_w / 2),
    ...
    def _explode(self):
        """ Starts the explosion animation that ends the Pawn's
            life.
        """
        self.state = Pawn.EXPLODING
        pos = ( self.pos.x - self.explosion_images[0].get_width() / 2,
                self.pos.y - self.explosion_images[0].get_height() / 2)
        self.explode_animation = SimpleAnimation(
            self.screen, pos, self.explosion_images,
...
def run_game():
    ...
    pawn_images = [
        pygame.image.load(filename).convert_alpha() 
            for filename in PAWN_FILENAMES]
    ...
    # Create N_PAWNS random creeps.
    pawns = pygame.sprite.Group()
    for i in range(N_PAWNS):
        pawns.add(
            Pawn(  screen=screen,
                    pawn_image=choice(pawn_images), 
                    explosion_images=explosion_images,
                    field=FIELD_RECT,
                    init_position=( randint(FIELD_RECT.left, 
                                            FIELD_RECT.right), 
