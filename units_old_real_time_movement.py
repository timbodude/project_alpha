...
class Unit(Sprite):
            ...
            if not self.in_combat:
                ...
                if self.wander:
                    ...
                    if not unit_vs_obstacle_collision_check(self):
                        ...
                        #print("I'm checking for obstacle collision.")
                        #check to see if occupied by obstacle (returns true = collision / false = no collision)                    
    
                        #print("no obstacle collision detected")
                        self.pos += displacement
                        self.rect[0] = self.pos[0]
                        self.rect[1] = self.pos[1]
                    else:
                        #print()
                        #print("obstacle collision detected. Angle:", self.direction.angle)
                        self.direction.angle += 180
                        if self.direction.angle > 360:
                            self.direction.angle -= 360
                        #print("Angle after collision:", self.direction.angle)
                        #print()
                ...
                ########### Rotate image if necessary & check for wall collision
    ...
    def unit_vs_unit_los_check(self, hero_group, enemy_unit, enemy_group, kill=False):
        ...
        if distance(self.pos, enemy_unit.pos) <= self.los:
            print("woohoo - enemy in los - distance =", distance(self.pos, enemy_unit.pos))
            ...
            self.wander = False
            a = self.pos[0] - enemy_unit.pos[0]
            b = self.pos[1] - enemy_unit.pos[1]
            a *= a
            b *= b
            d = int(math.sqrt(a + b))
            if self.pos[0] - enemy_unit.pos[0] == 0:
                self.pos[0] += .01
            m = (self.pos[1] - enemy_unit.pos[1]) / (self.pos[0] - enemy_unit.pos[0])
            r = math.atan(m)
            deg = int(math.atan(r)*100)
            
            #self.direction.angle = 0
            
            #if self.pos[0] + deg < enemy_unit.pos[0] and self.pos[1] + deg < enemy_unit.pos[1]:
                #if self.direction.angle < 180:
                    #self.direction.angle -= self.turn_to_opp
                #else:
                    #self.direction.angle += self.turn_to_opp   
            #elif self.pos[0] + deg < enemy_unit.pos[0] and self.pos[1] + deg > enemy_unit.pos[1]:
                #if self.direction.angle < 180:
                    #self.direction.angle += self.turn_to_opp
                #else:
                    #self.direction.angle -= self.turn_to_opp
...
class Creep(Unit):
    def __init__(self, screen):
        ...
        self.init_position = (randint(self.field.left, self.field.right), randint(self.field.top, self.field.bottom)/2) # top half screen
...
def unit_vs_obstacle_collision_check(unit):
    ...
    for group in ALL_OBSTACLES_GROUPS:
        ...
        for obstacle in group.group_list:
            ...
            if unit.pos[0] + (unit.rect[2]/2) >= obstacle.pos[0] - (obstacle.rect[2]/2) and unit.pos[0] - (unit.rect[2]/2) <= obstacle.pos[0] + (obstacle.rect[2]/2):
                if unit.pos[1] + (unit.rect[3]/2) >= obstacle.pos[1] - (obstacle.rect[3]/2) and unit.pos[1] - (unit.rect[3]/2) <= obstacle.pos[1] + (obstacle.rect[3]/2):  
                    #print("boing!")
                    return(True)
